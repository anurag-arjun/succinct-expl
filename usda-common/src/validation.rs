...
pub fn validate_batch(
    ...
    for tx in txs {
        if let Some(from_addr) = tx.from.as_ref() {
            let from_addr = from_addr.to_vec();
            let to_addr = tx.to.to_vec();
...
fn compute_message(tx: &crate::Transaction) -> Vec<u8> {
    let mut hasher = Sha256::new();
    if let Some(from_addr) = tx.from {
        hasher.update(&from_addr);
    }
    hasher.update(&tx.to);
